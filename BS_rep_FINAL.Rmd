...
trust$trust_scientists <- ifelse(is.na(trust$trust_medial_scientists),
                                 trust$trust_scientists,
                                 ifelse(is.na(trust$trust_scientists),
                                        trust$trust_medial_scientists,NA))
...
plot <- ggplot(loadings,aes(x = f1, y=f2,label=vars)) + 
  theme_minimal() + geom_label_repel() + scale_x_continuous("First Dimension Factor") + 
  scale_y_continuous("Second Dimension Factor") + 
  geom_segment(aes(x = 0,y = 0,xend = f1,yend = f2),arrow=arrow())  
# (Proportion of Variance F1: 18%) (Propoportion of F2 Variance 16%)"
...
plot <- ggplot(subset(mes,!is.na(mes$race3)),
  ...
  labs(caption="Note marginal effects relative to 
       white respondents. \n* p < 0.1; ** p < 0.05; *** p < 0.01") + 
  facet_wrap(~model) + scale_shape_manual("Trust Effects",values=c(23,22,21,20)) +    
  geom_text(aes(label= AME), size = 5, vjust= "top")
