...
covid_ideo_scales <- sapply(covid_ideo_scales, as.numeric)
...
# Run factor analysis
factanal <- fa(covid_ideo_scales[,2:9], nfactors=2, rotate="promax", fm="pa")
...
ggplot(data= na.omit(as.data.frame(cbind(pew_ATP_W64$female, pew_ATP_W64$covid_restriction_fa_dim1))),
...
#transforming political identification 
pew_ATP_W64$pid3 <- as.character(pew_ATP_W64$F_PARTY_FINAL)
#pew_ATP_W64$pid3 <- factor(pew_ATP_W64$pid3,levels=c("Republican","Independent","Democrat")) #this was making all the values NA for some reason
pew_ATP_W64$pid3 <- case_when(pew_ATP_W64$pid3 == 1 ~ "Republican",
                              pew_ATP_W64$pid3 == 2 ~ "Independent",
                              pew_ATP_W64$pid3 == 3 ~ "Democrat")

pew_ATP_W64$pid3 <- as.factor(pew_ATP_W64$pid3)
...
#did not work so used case_when
...
pew_ATP_W64$race3 <- as.factor(pew_ATP_W64$F_RACETHN)

pew_ATP_W64$race3 <- case_when(pew_ATP_W64$race3 == "1" ~ "white",
                                   pew_ATP_W64$race3 =="2" ~ "black",
                                   pew_ATP_W64$race3 == "3" ~ "hispanic")

sum(is.na(pew_ATP_W64$race3))

pew_ATP_W64$race3 <- ifelse(is.na(pew_ATP_W64$race3) & pew_ATP_W64$F_RACECMB %in% 3,"asian",pew_ATP_W64$race3)
...
#recoding manually into factor because loop above did not work
...
pew_ATP_W64_trust <- merge(pew_ATP_W64, trust, by =c("QKEY"), all=T)

x0 <- subset(pew_ATP_W64_trust,select=c(pid3,trust_scientists_fa_dim1,trust_scientists_fa_dim2))
x0 <- na.omit(x0)
...
new_row <- data.frame(pid3 = "Full Sample")

# add the new row to the data frame
x_ <- rbind(x_, new_row)

# update the factor levels
x_$pid3 <- factor(x_$pid3, levels = c("Republican", "Independent", "Democrat", "Full Sample"), 
                  labels = c("Republican Partisans", "Independent Partisans", "Democratic Partisans", "Full Sample"))
...
#making policy support variables binary again
pew_ATP_W64$restriction_intl_travel <- recode(pew_ATP_W64$restriction_intl_travel, 
                            "1"= 1, "2"= 0)  
pew_ATP_W64$restriction_most_business <- recode(pew_ATP_W64$restriction_most_business, 
                            "1"= 1, "2"= 0) 
pew_ATP_W64$restriction_large_gatherings <- recode(pew_ATP_W64$restriction_large_gatherings, 
                            "1"= 1, "2"= 0)
pew_ATP_W64$restriction_sporting_events <- recode(pew_ATP_W64$restriction_sporting_events, 
                            "1"= 1, "2"= 0) 
pew_ATP_W64$restriction_closing_k12 <- recode(pew_ATP_W64$restriction_closing_k12, 
                            "1"= 1, "2"= 0) 
pew_ATP_W64$restriction_carry_out_only <- recode(pew_ATP_W64$restriction_carry_out_only, 
                            "1"= 1, "2"= 0) 
pew_ATP_W64$restriction_postponing_primary <- recode(pew_ATP_W64$restriction_postponing_primary, 
                            "1"= 1, "2"= 0)

#running loop where model is run on each policy support variable w/ covariates
for(i in which(colnames(pew_ATP_W64) == "restriction_intl_travel"):which(colnames(pew_ATP_W64) == "restriction_postponing_primary")){
  #running model
  summary(model <- glm(pew_ATP_W64[,i] ~ trust_scientists_fa_dim1 + trust_media + 
                         trust_elected_officials + female + pid3 + libcon + age_linear + 
                         educ_linear + income_linear + race3 + region_factor, data=pew_ATP_W64, 
                       weights=as.numeric(weight), family = binomial(link = "logit")))
  #storing model margins
...
for(i in which(colnames(pew_ATP_W64) == "restriction_intl_travel"):which(colnames(pew_ATP_W64) == "restriction_postponing_primary")){
  #running model
  summary(model <- glm(pew_ATP_W64[,i] ~ trust_scientists_fa_dim1*race3 + 
                         trust_media + trust_elected_officials + female + pid3 + 
                         libcon + age_linear + educ_linear + income_linear + 
                         region_factor, data=pew_ATP_W64, weights=as.numeric(weight), 
...
```{r}
summary(model <- glm(covid_restriction_irt ~ trust_scientists_fa_dim1*race3 + 
                       trust_media*race3 + trust_elected_officials*race3 + female + 
                       libcon + age_linear + educ_linear + income_linear + 
                       region_factor, data=pew_ATP_W64, weights=as.numeric(weight), 
                     family = gaussian(identity)))

baseline_trust_effects.4 <- summary(margins(model, variables=c("trust_scientists_fa_dim1","trust_media","trust_elected_officials"),
                                            at=list(race3=c("asian","white","black",
                                                            "hispanic")), type="response", 
                                            change="minmax"))
...
summary(model <- glm(summated_restriction_scale ~ trust_scientists_fa_dim1*race3 + 
                       trust_media*race3 + trust_elected_officials*race3 + female + 
                       libcon + age_linear + educ_linear + income_linear + 
                       region_factor, data=pew_ATP_W64, weights=as.numeric(weight), 
                     family = gaussian(identity)))
...
plot <- ggplot(x, aes(x=race3,y=trust_scientists_fa_dim1, group=race3,fill=race3)) + 
  ...
  theme(legend.position = "none") + labs(caption="ANOVA suggests significant 
                                         differences in mean latent scientific trust 
                                         across racial groups, p < 0.01.") + 
...
# Data Analysis Figures: OLS Composite Models
```{r, eval=F}
summary(model <- lm(trust_scientists_fa_dim1 ~ female + pid3 + libcon + age_linear + 
                      educ_linear + income_linear + race3 + region_factor, data=pew_ATP_W64, 
                    weights=as.numeric(weight)))
...
mes$race3 <- factor(mes$factor,levels=c("race3asian","race3black","race3hispanic"),
                    labels=c("Asian Respondents","Black Respondents","Hispanic Respondents"))
...
plot <- ggplot(subset(mes,!is.na(mes$race3)),
               aes(x=race3,y=AME,factor=race3,group=race3,color=race3,shape=race3,
                   label=label,fill=race3)) + 
  facet_wrap(~model) + 
  coord_flip() + 
  ...
  labs(caption="Note marginal effects relative to white respondents. \n* p < 0.1; 
       ** p < 0.05; *** p < 0.01") + 
  facet_wrap(~model) + 
...
mes <- subset(mes,!(mes$factor %in% c("region_factorWest","region_factorSouth",
                                      "region_factorMidwest")))
...
plot <- ggplot(mes,aes(x=factor,y=AME,factor=factor,group=factor,label=label)) + 
  ...
  theme_minimal() + 
  scale_x_discrete("",labels=c("Age","Education","Female","Income",
                               "Liberal Ideology","Democrat","Independent",
                               "Asian Respondent","Black Respondent","Hispanic Respondent")) + 
  ...
  labs(caption="Note marginal effects for respondent race & 
       partisanship relative to baseline factor categories. Contextual regions 
       omitted. \nFactor Baselines: white respondents, 
       Republican identifiers. * p < 0.1; ** p < 0.05; *** p < 0.01") + 
  facet_wrap(~model) 
